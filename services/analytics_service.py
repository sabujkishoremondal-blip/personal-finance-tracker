"""Analytics: monthly reports, comparisons, budget progress."""
from datetime import date
from services.expense_service import (
    category_totals, monthly_totals, list_expenses, get_budgets,
)
from services.balance_service import total_expenses, month_summary
...
def compare_last_months(months: int = 6) -> dict:
    data = monthly_totals(months=months)
    if len(data) < 2:
        return {"data": data, "change_pct": None, "highest": None, "lowest": None, "avg": None}
    ...
    # biggest category driver in current vs prev
    cy, cm = map(int, data[-1]["ym"].split("-"))
    py, pm = map(int, data[-2]["ym"].split("-"))
    curr_cats = {c["category"]: c["total"] for c in category_totals(cy, cm)}
    ...
    return {
        "data": data, "change_pct": change_pct, "highest": highest,
        "lowest": lowest, "avg": avg, "biggest_driver": biggest_driver,
    }


def budget_progress(year: int, month: int) -> list:
    budgets = get_budgets(year, month)
    cats = {c["category"]: c["total"] for c in category_totals(year, month)}
    out = []
    for cat, budget in budgets.items():
        if cat == "TOTAL":
            spent = sum(cats.values())
        else:
