...
from services.balance_service import owed_by_friends, owed_to_others
...
def render():
    ...
    owe_them = owed_to_others(uid)
    owe_you = owed_by_friends(uid)
    ...
    with tab_bor:
        _render_list(list_debts(uid, direction="borrowed"), currency, "borrowed")
    with tab_lent:
        _render_list(list_debts(uid, direction="lent"), currency, "lent")
    with tab_add:
        _add_form(currency)
...
def _add_form(currency):
    ...
    with st.form("add_debt_form", clear_on_submit=True):
        ...
        c1, c2 = st.columns(2)
        with c1: person = st.text_input("Person")
        with c2: amt = st.number_input("Amount", min_value=0.0, step=1.0, format="%.2f")
        ...
        if st.form_submit_button("Save", use_container_width=True):
            if not person.strip() or amt <= 0:
                ...
                add_debt(uid, d_val, person.strip(), amt, reason, dt)
                st.success("Saved.")
                st.rerun()
