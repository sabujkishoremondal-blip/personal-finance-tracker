...
from components.cards import transaction_card, empty_state
...
def _cat_pretty(name: str) -> str:
    return f"{category_icon(name)}  {name}"
    ...
    with st.form(f"{key_prefix}_expense_form", clear_on_submit=True):
        ...
        with col2:
            ...
            cat = st.selectbox("Category", opts,
                               format_func=_cat_pretty, index=idx, key=f"{key_prefix}_cat")
        ...
        if submitted:
            if amt <= 0:
                ...
                return None
            eid = add_expense(amt, cat, desc, dt, merchant,
                              source=prefill.get("source", "manual"),
                              screenshot_path=prefill.get("screenshot_path", ""))
            st.success(f"Added {format_money(amt, get_setting('currency', 'INR'))}. "
                       f"Balance: {format_money(available_balance(), get_setting('currency', 'INR'))}")
            return eid
    return None
...
def _add_balance_form():
    with st.form("add_balance_form", clear_on_submit=True):
        ...
        submitted = st.form_submit_button("Add Balance", use_container_width=True)
        if submitted:
            if amt <= 0:
                st.error("Amount must be greater than 0.")
                return
            add_balance(amt, src, desc, dt)
            st.success(f"Added {format_money(amt, get_setting('currency', 'INR'))} to balance.")
            st.rerun()
...
def _ocr_flow():
    ...
    if not ocr_service.is_available():
        ...
        return
    up = st.file_uploader("Upload PNG / JPG / WEBP", type=["png", "jpg", "jpeg", "webp"],
                          key="ocr_upload")
    if up:
        try:
            ...
            return
        st.image(img, use_column_width=True, caption="Uploaded screenshot")
        ...
        with c1:
            st.markdown(f"**Amount**  \n{format_money(prefill['amount'] or 0, get_setting('currency','INR'))}")
        with c2:
...
def render():
    ...
    # honor navigation intents from dashboard
    if st.session_state.pop("exp_open_ocr", False):
        st.session_state["active_exp_tab"] = 2
    if st.session_state.pop("exp_open_addbal", False):
        st.session_state["active_exp_tab"] = 1
    if st.session_state.pop("exp_open_add", False):
        st.session_state["active_exp_tab"] = 0
    ...
    with tab_bal:
        with st.container():
            st.markdown('<div class="pf-card">', unsafe_allow_html=True)
            st.markdown(f"**Current balance:** {format_money(available_balance(), currency)}")
            _add_balance_form()
            st.markdown("</div>", unsafe_allow_html=True)
...
def _history_view(currency):
    ...
    with f2:
        sort = st.selectbox("Sort by", ["Newest", "Oldest", "Highest", "Lowest"],
                            key="hist_sort")
    with f3:
        month_str = st.text_input("Month (YYYY-MM)", value="", key="hist_month",
                                  placeholder="2026-09")
    with f4:
        search = st.text_input("Search", key="hist_search", placeholder="lunch, uber…")
    ...
    items = list_expenses(year=y, month=m, category=cat, search=search or None,
                          sort=sort_map[sort])
    if not items:
        ...
        with st.container():
            ...
            with cols[1]:
                st.markdown(
                    ...
                    f"{e['category']} · {e['date']}"
                    f"{' · via screenshot' if e['source']=='screenshot' else ''}</span>",
                    unsafe_allow_html=True,
                )
            with cols[2]:
                st.markdown(
        ...
        if target:
            with st.container():
                ...
                with st.form(f"edit_form_{eid}"):
                    ...
                    dt = st.date_input("Date", value=datetime.fromisoformat(target["date"]).date())
                    save, cancel = st.columns(2)
                    if save.form_submit_button("Save", use_container_width=True):
                        update_expense(eid, amount=amt, category=cat_new,
